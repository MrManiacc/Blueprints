...
object Network {
    ...
    private var ID = 0
    val serverRegisteredBlocks = HashSet<BlockPos>()
    val clientRegisteredBlocks = HashSet<BlockPos>()
                            ...
                            if (serverCallbacks.containsKey(type)) {
                                ...
                                for (method in methodList)
                                    if (method.first.name.equals(it.name) && method.second == any) {
                                        ...
                                        info("successfully removed method callback ${it.name} on the server from class $any from ${method.second}")
                                        break
                                    }
                            }
                        }
                        LogicalSide.CLIENT -> {
                            if (clientCallbacks.containsKey(type)) {
                                ...
                                for (method in methodList)
                                    if (method.first.name.equals(it.name) && method.second == any) {
                                        methodList.remove(method)
                                        info("successfully removed method callback ${it.name} on the client $any from ${method.second}")
                                        break
                                    }
                            }
                        }
