 ...
 */
class NodeWorldData(val world: ServerWorld) : WorldSavedData(DATA_NAME) {
    ...
    /**
     * This will remove the graph. It should only be called from the server when the block is destoryed.
     */
    fun remove(uuid: UUID): Boolean {
        return graphs.remove(uuid) != null
    }
