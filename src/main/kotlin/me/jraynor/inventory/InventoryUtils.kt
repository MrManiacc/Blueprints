...
object InventoryUtils {
    ...
    /**
     * This will try to move the given count from the src to the dest
     */
    fun tryMoveNextItem(
        src: IItemHandler,
        dest: IItemHandler,
        count: Int,
        filter: IFilter<ItemStack, IItemHandler>? = null
    ): ItemStack {
        var stack = EMPTY
        for (slot in 0 until src.slots) {
            stack = src.extractItem(slot, count, true)
            if (filter != null) {
                val result = filter.filter(stack, src, dest)
                if (!filter.filter(stack, src, dest).value)
                    continue //If the filter fails we skip this slot.
            }
            stack = src.extractItem(slot, count, false)
            if (stack.isEmpty)
                continue //Nothing was extracted from the given slot
            if (canAccept(stack, dest))
                stack = ItemHandlerHelper.insertItemStacked(dest, stack, false)
            else {
                ItemHandlerHelper.insertItemStacked(src, stack, false)
                continue
            }
            if (stack.isEmpty) return stack
        }
